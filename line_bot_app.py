...
# 📸 Renderサーバーから本物画像を配信するURLエンジン
def get_dynamic_image_url(data, base_url):
    pic_name = data.get('PicFileName', '').strip()
    if not pic_name or pic_name.lower() in ['nan', 'none', 'null', '']:
        return None
    
    if pic_name.startswith('http'):
        return pic_name
        
    clean_base = base_url if base_url.endswith('/') else base_url + '/'
    return f"{clean_base}static/images/{pic_name}"
...
def build_single_bubble(photo_id, item, base_url, title_text="📌 旬の厳選おすすめ撮影地"):
    ...
    img_url = get_dynamic_image_url(item, base_url)
    if img_url:
    ...
    bubble_body_contents.extend([
        {"type": "text", "text": title_text, "size": "xs", "color": "#e74c3c", "weight": "bold", "margin": "lg" if img_url else "none"},
    ...
    return {
        ...
        "body": {"type": "box", "layout": "vertical", "paddingAll": "xl", "contents": bubble_body_contents}
    }
...
def build_location_detail_card(photo_id, data, current_db, base_url):
    ...
    img_url = get_dynamic_image_url(data, base_url)
    if img_url: card_json["hero"] = {"type": "image", "url": img_url, "size": "full", "aspectRatio": "16:10", "aspectMode": "cover"}
    return card_json
...
def handle_line_message(event):
    """【真のセンターピンスナイプ】時期ウィンドウ（Month×Period）で最初からピンポイント直接ロード"""
    ...
    # 🗓️ 明日を起点とした前5日・後10日（計16日間）の「半月ウィンドウ」ペアを厳密に算出
    parsed_periods = []
    for i in range(-5, 11):
    ...
    parsed_periods = list(set(parsed_periods)) # 集約
    ...
    # 💥 【Kaoru设计図通り：半月ウィンドウ・ピンポイント・スナイプ】
    precise_seasonal_pool = []
    try:
        for m_val, p_val in parsed_periods:
            for m_str in [str(m_val), f"{m_val:02d}"]: # 表記揺れ防衛
                docs = ref.where('Month', '==', m_str).where('Period', '==', p_val).stream()
                for doc in docs:
                    precise_seasonal_pool.append((doc.id, doc.to_dict()))
    except Exception as e: print(f"Firestore Precise Snipe Error: {e}", flush=True)

    main_pool = []
    radius_pool = []

    # 3. 🎯 ロードされた「その半月分のジャストな塊」だけを250km圏内で超高速精査
    ...
    # キーワードマッチ優先での並び替え（全データの中で最良の3件を厳選）
    ...
    # 💥 【最後の盾】全滅時も霧ヶ峰photo_0ではなく、photo_randomから拾い上げてデモを繋ぐ（データ無視を扑灭）
    if not main_pool:
    ...
    if intent_pref:
        greeting = f"ようこそ『風景写真』コンシェルジュ the 部屋へ。明日（{base_date.strftime('%m/%d')}）撮影にお出かけですか。{weather_phrase}撮影を楽しめそうですね。今の季節にまさに『旬』を迎えているおすすめのポイントをご案内いたします。"
    else:
